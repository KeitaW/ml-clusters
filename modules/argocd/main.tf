...
resource "kubernetes_secret_v1" "hub_cluster" {
  metadata {
    ...
    labels = merge(
      ...
      { for k, v in var.hub_annotations : k => v if contains(["enable_karpenter", "enable_external_dns", "enable_adot", "enable_kuberay"], k) }
    )
    annotations = merge(
      {
...
resource "kubernetes_secret_v1" "spoke_clusters" {
  ...
  metadata {
    ...
    labels = merge(
      ...
      { for k, v in each.value.annotations : k => v if contains(["enable_karpenter", "enable_external_dns", "enable_adot", "enable_kuberay"], k) }
    )
    annotations = merge(
      {
