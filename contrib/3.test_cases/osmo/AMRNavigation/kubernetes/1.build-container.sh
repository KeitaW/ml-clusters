...
# Also support legacy single-image build
REPO_NAME="${ECR_REPO_NAME:-isaac-sim-mobilitygen}"
IMAGE_URI="${ACCOUNT_ID}.dkr.ecr.${REGION}.amazonaws.com/${REPO_NAME}:${IMAGE_TAG}"

echo "=== Building MobilityGen containers ==="
echo "  Region:    ${REGION}"
echo "  Account:   ${ACCOUNT_ID}"
echo ""
echo "  Images:"
echo "    Legacy:        ${IMAGE_URI}"
...
for repo in "${REPO_NAME}" "${ISAAC_SIM_REPO}" "${COSMOS_REPO}" "${XMOBILITY_REPO}"; do
    aws ecr describe-repositories --repository-names "${repo}" --region "${REGION}" 2>/dev/null || \
        aws ecr create-repository --repository-name "${repo}" --region "${REGION}"
done

# Build legacy single-stage image (backward compatible)
echo ""
echo "--- Building legacy MobilityGen image ---"
